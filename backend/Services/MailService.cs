...
namespace backend.Services
{
    public class MailService
    {
        ...
        public void SendContact(Contact contact)
        {
            ...
            var serialized = JsonConvert.SerializeObject(settings);
            var contactMail = JsonConvert.DeserializeObject<ContactMail>(serialized);

            contactMail.TemplateId = 7;

            contactMail.Params = new Models.Mails.Contact.ContactParams
            {
                Company = contact.Company,
                Name = contact.Name,
                Phone = contact.Phone,
                Email = contact.Email,
                Machine = contact.Machine?.MachineName,
            ...
            contactMail.To = emailAddresses;

            var jsonContent = JsonConvert.SerializeObject(contactMail);
